...
impl<'a, Meta: Default + Sync> ExecutorThread<'a, Meta> {
    ...
    fn update_binode(
        ...
        if data.phase == BiPhase::Init {
            ...
            if both_stages {
                data.arr0 =
                    nine_children_overlapping(&self.mem, n0.nw, n0.ne, n0.sw, n0.se);
                data.arr1 =
                    nine_children_overlapping(&self.mem, n1.nw, n1.ne, n1.sw, n1.se);
                data.mask9_waiting = 0b1_1111_1111;
            } else {
    ...
    fn start_solitonic(
        ...
        match handle_dependency(n0, tagged_dep) {
            ...
            DependencyHandlingResult::StartedByThisThread => {
                self.queue
                    .push(Task::Node { idx: idx.0, size_log2 });
                waiting_cnt += 1;
            }
            DependencyHandlingResult::StartedByOtherThread => {
                waiting_cnt += 1;
        ...
        match handle_dependency(n1, tagged_dep) {
            ...
            DependencyHandlingResult::StartedByThisThread => {
                self.queue
                    .push(Task::Node { idx: idx.1, size_log2 });
                waiting_cnt += 1;
            }
            DependencyHandlingResult::StartedByOtherThread => {
                waiting_cnt += 1;
    ...
    fn finish_solitonic(
        ...
        if orig0 == b || orig1 == b {
            ...
            let lanes = engine.node2lanes(ind3, size_log2);
    ...
    fn finish_base_case(
        ...
        if i3 != b {
            let lanes = engine.node2lanes(hnode2, size_log2);
            if lanes & 0xf0 != 0 {
                (b, i3)
            } else {
                (i3, b)
